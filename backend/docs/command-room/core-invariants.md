...
4. **Round cannot become Jira, dashboard, or gate.** A Round is factual working memory for the lead AI, not a user-facing task tracker, fixed form flow, automatic PASS/FAIL engine, or program-controlled review pipeline.
...
- These anchors require AI checking and opposition without adding a runtime gate, program-selected reviewer, PASS/FAIL label, or automatic rework loop.
