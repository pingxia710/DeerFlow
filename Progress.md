...
- 稳定性补证完成：两次独立的真实 `codex exec --json`（`gpt-5.6-terra`、read-only、固定无工具提示词）分别形成 source Run `7920aa1a-…` 与 `3f519f52-…`。两者均 success，并分别持久化 `subagent_tokens/total_tokens` `26,331` 与 `26,327`；每行的 `gpt-5.6-terra` 分桶与终态 JSONL 严格一致（input 都为 `26,299`，output 分别 `32`、`28`）。连同 Command Room `LEDGER_PROBE`，完成三份独立真实 usage → Run Ledger 证据。未知/缺失/畸形 JSONL 继续安全降级为不计数，子任务结果与唤醒不受影响；既有 `/token-usage` API 和 `TokenUsageIndicator` 已直接消费同一账本，不需新增数据合同或 UI 状态。
- 兼容降级补测：`test_completion_usage_ignores_unsupported_or_malformed_jsonl_events` 固定未知事件、坏 JSON 与非整数/布尔 usage 的不计数语义；transport、RunManager、SQL/内存账本与分桶聚焦回归共 `210 passed`，Ruff 通过。
