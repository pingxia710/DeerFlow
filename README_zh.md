...
前提：先完成上面的“配置”步骤（`make config` 和模型 API key 配置）。`make dev` 需要有效配置文件，默认读取项目根目录下的 `config.yaml`。可以用 `DEER_FLOW_PROJECT_ROOT` 显式指定项目根目录，也可以用 `DEER_FLOW_CONFIG_PATH` 指向某个具体配置文件。本源码仓库的运行期状态默认写到 `backend/.deer-flow`；独立嵌入项目默认写到项目根目录下的 `.deer-flow`。两者都可用 `DEER_FLOW_HOME` 覆盖。`make doctor` 会报告运行数据是否分裂在多个根目录。skills 默认读取项目根目录下的 `skills/`，可用 `DEER_FLOW_SKILLS_PATH` 覆盖。
...
Web 对话会将每次用户请求与对应的 AI 工作保持在同一轮中；回复流式生成或加载更早历史时，阅读位置不会被打断，并会在可用时显示简洁的完成时间。
...
lead agent 通过 `task()` 拉起一次性 sub-agent。每次调用把包含开发者所写专业角色上下文的完整自然语言 prompt 交给一个临时 Codex CLI AI；Codex 自主规划和选择原生工具，返回完整自然结果后退出。由 lead AI 根据目标和上下文决定派发多少个有价值的任务；DeerFlow 不会用程序截断、排队、分批或延后 `task()` 调用。
...
在 `command-room` 运行中，DeerFlow 运行时之上的 AI 组织产品层是 **NextOS**。`command-room` 继续作为内部 Agent 兼容标识，用户看到的指挥室身份和大企业团队运行模型则是 NextOS。指挥室是逻辑常驻的脑袋，在连续 Run 之间保留目标、已接受方案、进度、上下文和最终判断；每个专业子 AI 只为一个自包含任务临时创建，返回完整自然结果后结束。

实质性工作遵循由 AI 掌握的“`planner` 提案 → 一轮 `opposition` 反方挑战 → Chair 执行方案 → 人确认方案 → 执行 → 方案完成”。反方在完整方案出来后运行，同时获得原始目标、事实、约束、标准和完整方案，检查隐藏假设、反证、失败路径和更强替代方案；它不为反对而反对，也不代替 Chair 决策。合成后若核心方向改变，对新方案再跑一轮反方；局部修订不重复。需要探索多条路线时，可以并行启动额外 Planner，但这属于并行方案探索，不属于反方。

执行前，Chair 与人讨论合成后的方案，等待人用自然语言明确授权执行。执行中，Chair 直接阅读当前文件、代码、日志、方案、工件和返回的任务结果，把它们作为事实；在已确认方案内自行处理普通实现细节、继续推进，不做逐任务验收，也不要求 `verifier` 角色。只有事实要求改变核心方向、范围、权限或业务决策时，才回到人与方案的讨论。角色是按需创建的临时专业视角，不是常驻进程。AI 之间通过完整自然语言 prompt 和完整结果交接；程序不选择角色、不评价结果、不执行阶段门禁、不触发返工，也不决定下一步。

NextOS 让组织长期存在，但不让模型进程常驻。它围绕目标按需组建专业 AI 团队，默认由根 Chair 完成 AI 之间的交接；只有大型目标确实需要独立协调上下文时，才建立一个有明确目标和边界的临时工作流负责人，完整结果返回根 Chair 后结束。完整运行契约保存在 [`skills/custom/nextos-commander/SKILL.md`](skills/custom/nextos-commander/SKILL.md)。

用户看到的 Command Room 回复保持自然、直接、面向行动。内部只保存线程、Run、任务、时间、引用和传输结果等客观事实，用于展示与传输恢复；这些记录不是 AI 协作本身，也不具有决定 AI 工作的权力。
...
**上下文监视器**：每次模型调用都会在真正的调用边界保存 owner 隔离的完整消息载荷和工具定义，并记录字符数、消息数和工具数。右上角“上下文”以字符数为主，而不是用 sub-agent 的 token 消耗代替；选择任意一轮后会按需读取并原样显示未截断的完整文本。升级前只有统计的旧快照仍可查看，但会明确提示当时没有保存全文。
