...
- **AI-AI text contract** — a `task` handoff is natural-language input to one `codex exec` worker, and its returned natural-language result must reach the lead AI in full. Keep `task` exempt from generic tool-output preview/truncation. DeerFlow owns only task lifecycle, timeout, cancellation, audit, result delivery, and the explicitly configured child model/effort; Codex CLI owns the model/tool loop. Record each actual lead model-call message/tool payload as an owner-scoped, non-truncated context event and expose it on demand in the top-right Context inspector; use character counts for this view, not subagent token spend. The Command Room Loop is inter-agent: one worker returns before the lead decides whether another AI is useful.
