...
- Treat the handoff packet as the core AI-AI governance unit. Pass a narrow goal, current round boundary, inherited/current context, required inputs, forbidden changes, released capabilities/tools/model/skill, expected outputs, evidence requirements, and failure/stop/escalation conditions; write prompts for the actual understanding limits of the receiving one-shot sub-AI. Audit may retain only compact extracted fields such as `context`, `requiredInputs`, `expectedOutput`, `releasedCapabilities`, and `stopConditions`, never raw prompts.
