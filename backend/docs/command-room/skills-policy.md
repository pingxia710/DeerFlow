...
- Command Room is the main development AI, not a workflow system. The user controls direction: the current round goal, boundaries, and whether to enter another round. Within that confirmed round, final execution judgment lives in the lead AI brain, not in workers, an always-on reviewer pipeline, or program-level gates.
