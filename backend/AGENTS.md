...
| **Threads** (`/api/threads/{id}`) | `DELETE /` - remove DeerFlow-managed local thread data after LangGraph thread deletion; best-effort cancels owner-scoped active runs and clears their local stream buffers before deleting persisted runs/events/metadata; unexpected failures are logged server-side and return a generic 500 detail |
