...
10. `LoopDetectionMiddleware` — breaks repetitive tool call loops for ordinary lead agents; excluded from the Command Room
...
The coordinator-only Command Room does not load this middleware. Its lead AI decides which worker, checker, or opposition AI runs next and when the work is complete.
...
**Configuration**: controlled by the `loop_detection:` section in `config.yaml`; never loaded by the Command Room.
...
Limits the number of subagent `task()` calls the agent can emit in one model response. This enforces the configured resource bound without creating a task queue or deciding what work the AI should issue next.
