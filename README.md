...
The lead agent delegates execution through `task()`. Each call sends one self-contained natural-language prompt, including developer-authored professional role context, to one ephemeral Codex CLI AI. Codex chooses its own plan and native tools, returns its complete natural-language result unchanged, and exits. The lead AI chooses how many useful tasks to dispatch; DeerFlow does not truncate, queue, batch, or defer task calls programmatically.
