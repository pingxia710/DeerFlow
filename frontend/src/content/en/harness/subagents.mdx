...
Each `task` handoff writes a compact append-only audit record under the thread's audit directory when local storage is available. The record keeps dispatch metadata, status, usage, hashes, character counts, and compact action-result facts, while omitting raw prompts, worker output, and error text. It does not infer a verdict, evidence strength, or next action from worker prose.
