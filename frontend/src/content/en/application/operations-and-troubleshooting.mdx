...
Every Gateway response includes `X-Request-ID`. Pass your own value when
reproducing an issue so proxy, frontend, and Gateway logs can be correlated:

```bash
curl -H "X-Request-ID: debug-$(date +%s)" http://localhost:8001/health
```

Gateway access logs include one JSON line per request with `event: "gateway.request"`,
`request_id`, `method`, `path`, `status_code`, and `duration_ms`.

Gateway also exposes Prometheus text metrics at `/metrics`; normal Gateway
authentication applies. Start dashboards with `deerflow_gateway_http_requests_total`,
`deerflow_gateway_http_request_duration_seconds_sum`, and
`deerflow_gateway_uptime_seconds`.
Useful first alerts are sustained 5xx request rate, rising average latency
(`duration_sum / requests_total`), and missing scrapes.
...
```bash
tar -czf deerflow-backup-$(date +%Y%m%d%H%M%S).tgz backend/.deer-flow
```
...
```bash
make stop
mv backend/.deer-flow backend/.deer-flow.rollback.$(date +%Y%m%d%H%M%S)
tar -xzf deerflow-backup-YYYYMMDDHHMMSS.tgz
make dev
curl http://localhost:8001/health
```

Keep `.env`, API keys, and OAuth/session secrets out of shared backup artifacts unless the backup target is encrypted and access-controlled.
