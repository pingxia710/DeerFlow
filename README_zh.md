...
指挥室默认在自身上下文中直接执行范围内工作。只有任务确实需要独立上下文、并行、独立视角或更多上下文时，才通过 `task()` 拉起一次性 sub-agent。每次任务调用把包含开发者所写专业角色上下文的完整自然语言 prompt 交给一个临时 Codex CLI AI；Codex 自主规划和选择原生工具，返回完整自然结果后退出。
...
实质性工作遵循由 AI 掌握的“Chair 起草方案 → 一轮 `opposition` 反方挑战 → 人确认方案 → 执行 → 方案完成”。反方在完整方案出来后运行，同时获得原始目标、事实、约束、标准和完整方案，检查隐藏假设、反证、失败路径和更强替代方案；它不为反对而反对，也不代替 Chair 决策。合成后若核心方向改变，对新方案再跑一轮反方；局部修订不重复。需要探索多条路线时，可以并行启动额外 Planner，但这属于并行方案探索，不属于反方。

执行前，Chair 与人讨论合成后的方案，等待人用自然语言明确授权执行。执行中，Chair 默认亲自使用已配置工具完成工作，并始终掌握目标、优先级和下一步指挥权；只有独立执行上下文确有价值时才派发一次性专业 AI。在已确认方案内自行处理普通实现细节、继续推进，不做逐任务验收，也不要求 `verifier` 角色。只有事实要求改变核心方向、范围、权限或业务决策时，才回到人与方案的讨论。角色是按需创建的临时专业视角，不是常驻进程。AI 之间通过完整自然语言 prompt 和完整结果交接；程序不选择角色、不评价结果、不执行阶段门禁、不触发返工，也不决定下一步。
