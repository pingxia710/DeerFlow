...
- Feedback create/list/delete routes are owner-bound user data; resolve the owner with `get_request_storage_user_id(request)` so trusted internal channel calls do not write feedback under the synthetic internal/default user.
...
- `RunStore.list_inflight()` is the startup recovery source and must use the same inflight status set; do not limit it to only `pending`/`running`.
...
**Command-room audit**: `command-room` runs may append compact internal records to `audit/command_room_rounds.jsonl`. This is backend evidence for debugging and probes only; it must not drive visible chat UI or force user-facing response format. API reads must resolve the owner from the request storage owner so trusted internal channel calls read the bound user's round ledger, not the synthetic internal/default bucket.
