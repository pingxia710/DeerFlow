...
在 `command-room` 运行中，DeerFlow 遵循 AI-AI-AI。指挥室是脑袋，保留目标、进度、上下文、边界和最终裁决；一次性专业 sub-agent 根据完整 prompt 执行工作、写入 Markdown 交接并返回完整自然结果。普通交流不启用容器；当人的目标、边界和完成标准已经明确时，直接进入执行。只有方向或方案需要收敛时，才由正向与反向两个独立 AI 从同一份指挥室 brief 出发，分别写入 `01-planning/`；指挥室综合后把唯一方案记录为 `spec.md`。涉及架构、接口、数据、自动化、安全等重大技术取舍时，再按同样方式形成可选的 `02-technical-design/technical-plan.md`。

每次真实执行都使用 `Execution N → Review N`：执行 AI在真实工作区落地并记录变更、证据和限制；另一个 AI按任务目标独立审查实际结果，把自然语言 findings 写入 `03-delivery/cycle-NN/review/`。简单文件操作只做必要的事实核对，代码工作则检查目标与改动、行为、测试、回归和边界。指挥室阅读审查原文后结束任务，或显式发起下一执行周期；没有固定轮数，程序也不会解析 findings、自动判定或自动返工。运行时只记录声明的阶段/周期、路径、哈希、大小、生命周期和文件是否改写等客观事实。

用户看到的 Command Room 回复保持自然、直接、面向行动；内部审计只是运行记录，不是 AI 协作本身或质量仪表盘。每个子 AI 完成后都会结束，结果通过 prompt、自然语言和共享 Markdown 交给下一位 AI 或指挥室，而不是由程序替 AI 做判断。
