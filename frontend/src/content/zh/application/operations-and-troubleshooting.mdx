...
Gateway 每个响应都会带 `X-Request-ID`。复现问题时可以自己传入，方便把代理、前端和 Gateway 日志串起来：

```bash
curl -H "X-Request-ID: debug-$(date +%s)" http://localhost:8001/health
```

Gateway access log 每个请求会写一行 JSON，包含 `event: "gateway.request"`、
`request_id`、`method`、`path`、`status_code` 和 `duration_ms`。

Gateway 也在 `/metrics` 暴露 Prometheus text metrics；它走正常 Gateway
认证。Dashboard 可先接 `deerflow_gateway_http_requests_total`、
`deerflow_gateway_http_request_duration_seconds_sum` 和
`deerflow_gateway_uptime_seconds`。
第一批告警建议只看持续 5xx 请求率、平均延迟上升
（`duration_sum / requests_total`）和 scrape 缺失。
...
除非备份目标已加密且有访问控制，不要把 `.env`、API key、OAuth/session secrets 放进共享备份包。
