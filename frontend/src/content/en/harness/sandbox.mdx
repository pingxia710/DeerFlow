---
title: Sandbox
description: The sandbox gives ordinary agents and delegated workers a controlled environment for file and command work. The Command Room delegates execution through one-shot Codex tasks instead of using sandbox tools directly.
---
...
The sandbox gives ordinary agent profiles a controlled environment where they can read files, write outputs, run shell commands, and produce artifacts. The Command Room profile is different: it passes paths and boundaries to one-shot Codex sub-AIs through `task()` and does not use sandbox file or shell tools in the lead context.
...
Even without `bash`, an ordinary agent profile can still read and write files through the dedicated file tools. The Command Room delegates both file and shell work through `task()`.
...
`SandboxAuditMiddleware` inspects `bash` tool calls and records classified command audit events. It does not observe every dedicated file-tool read or write, so treat it as command auditing rather than a complete filesystem access log.
