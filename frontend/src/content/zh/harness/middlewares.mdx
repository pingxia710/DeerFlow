...
10. `LoopDetectionMiddleware` — 打破普通 Lead Agent 的重复工具调用循环；指挥室不加载
11. 自定义中间件（如有）
...
仅负责协调的指挥室不加载该中间件。由指挥室 Lead AI 决定下一个 worker、核对 AI 或反方 AI，以及何时完成工作。
...
**配置**：由 `config.yaml` 的 `loop_detection:` 段控制；指挥室始终不加载。
...
限制 Agent 在一次模型响应中最多可以发出的子 Agent `task()` 调用数量。它只执行配置的资源边界，不创建任务队列，也不替 AI 决定下一步工作。
