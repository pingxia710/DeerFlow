...
The Command Room executes in-scope work directly by default. It uses `task()` only when independent context, parallelism, a separate perspective, or more context is genuinely useful. Each task call sends one self-contained natural-language prompt, including developer-authored professional role context, to one ephemeral Codex CLI AI. Codex chooses its own plan and native tools, returns its complete natural-language result unchanged, and exits.
...
For `command-room` runs, the AI-organization product layer is **NextOS**, built on the DeerFlow runtime. `command-room` remains the stable internal agent identifier, while NextOS is the user-facing Command Room and enterprise-team operating model. The Chair is the logically resident brain: it keeps the goal, accepted plan, progress, context, and final judgment across sequential runs while every professional sub-AI is created for one self-contained task, returns its complete natural result, and ends.

Substantive work follows an AI-owned Chair draft plan → one `opposition` challenge → human plan confirmation → execution → plan completion flow. Opposition runs after the proposal and receives the original brief, facts, constraints, criteria, and complete plan. It exposes hidden assumptions, counterevidence, failure modes, and stronger alternatives without forcing disagreement or making the decision. If synthesis changes the core direction, the changed plan gets another opposition round; local refinements do not. Multiple independent proposals may run in parallel as additional planning perspectives, not as opposition.

Before execution, the Chair discusses its synthesized plan with the human and waits for explicit natural-language authorization to execute it. During execution, the Chair works directly by default and retains command over objectives, priorities, and next actions; it delegates only when a separate execution context is genuinely valuable. It resolves ordinary implementation details and continues the accepted plan without task-level acceptance or a required verifier role. It returns to human plan discussion only when facts require a core direction, scope, authority, or business-decision change. Roles are temporary professional perspectives rather than resident workers. Complete natural-language prompts and results are the AI-to-AI handoff; no program chooses a role, judges a result, enforces a stage, requires rework, or decides what happens next.

NextOS keeps the organization persistent without keeping model processes resident. It forms professional teams on demand, routes handoffs through the root Chair by default, and may create a temporary bounded workstream lead only when a large objective genuinely needs a separate coordination context. That lead returns its complete result to the root Chair and ends. The operating contract is preserved in [`skills/custom/nextos-commander/SKILL.md`](skills/custom/nextos-commander/SKILL.md).

DeerFlow can keep compact, owner/thread-scoped factual records for transport recovery without exposing them as a chat workflow. Run groups and task records preserve identities, current user text, references, transport outcomes, errors, and observed actions. A follow-up starts from the new user intent rather than treating an earlier answer or metadata as authority.
...
**Context Usage Monitor**: Each model call records the complete owner-scoped message payload and tool definitions at the actual model boundary, together with character/message/tool counts. The top-right Context view uses characters—not subagent token spend—as its primary measure and loads any selected call's full, non-truncated text on demand. Legacy metric-only snapshots remain readable but are clearly marked when full text was never stored.
