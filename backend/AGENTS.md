...
- `sandbox.unrestricted_host_access` is a local-only escape hatch for fully trusted single-user workflows: when true, LocalSandboxProvider tools may use direct host paths, bypass virtual path limits, and keep host paths visible in tool output. Keep the default false in shared, production, or untrusted contexts.
