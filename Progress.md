...
## 2026-07-13 - Direct Codex CLI task execution

- Replaced the production `task()` execution path from `SubagentExecutor`'s child LangGraph/model/tool loop and five-second polling with one `codex exec` process per task. DeerFlow now records lifecycle/audit events, applies the 60-minute outer timeout, terminates the process on cancellation, and returns the final message; Codex CLI owns planning, tool selection, model calls, and completion.
- `subagent_type` remains an audit/display label without selecting a DeerFlow role, tool allowlist, Skill set, or model-call limit. An existing `subagents.agents.<name>.model` override and global `subagents.reasoning_effort` now pass through to Codex CLI so the configured child model remains independent from the lead. The task process runs in the thread workspace using Codex CLI's `workspace-write` sandbox and an ephemeral session.
- The default `subagents.timeout_seconds` is now `3600`. Per-role model, reasoning-effort, Skill, and turn-limit settings no longer control the direct task path.

## 2026-07-14 - Freeze the DeerFlow AI-AI-AI execution contract

- Replaced the repository-level `What is DeerFlow` section with the developer-authored Chinese definition verbatim and marked that block developer-maintained. The root, backend, and frontend instruction chains remain below the 32 KiB combined budget. This definition is now the Goal Lock for the Command Room, child AIs, roles, checking, opposition, and final adjudication.
- Made the Command Room a coordinator-only brain. It retains the user goal, plan, progress, context, and final judgment; it has no direct file, shell, web, MCP, or ACP execution tools. Every investigation, implementation, modification, validation, and delivery action is delegated through `task()`. Every worker's complete natural result must go to a different checking/review/acceptance AI and to an independent opposition AI before the Command Room adjudicates, including small tasks.
- Removed program control from that collaboration path. The runtime no longer runs a child LangGraph/tool-turn loop, polls a child, infers quality/evidence/completion, selects the next role, dispatches review or rework, or manufactures PASS/FAIL, risk, gap, or recommendation text. Compatibility records retain factual state only. `LoopDetectionMiddleware` remains available to ordinary lead agents but is explicitly absent from the Command Room, so it cannot delete repeated `task()` calls or force the lead AI to stop before checking and opposition complete.
- Each `task()` now starts one isolated `codex exec --ephemeral` process, sends one self-contained natural-language prompt on stdin, returns the complete final message unchanged, and ends. `subagent_type` now selects only developer-maintained role prompt context (and an explicit model override when configured); this supersedes the previous entry's statement that it was only an audit/display label. It does not select a DeerFlow tool allowlist, Skill runtime, turn loop, model-call limit, recursion limit, checker, or next action. The prompt includes the goal, relevant paths and boundaries, and tells Codex to locate and read the applicable `AGENTS.md` chain.
- Set the child default independently from the lead: `gpt-5.6-terra`, native Codex `model_reasoning_effort="xhigh"`, and a 3600-second process timeout. The parent no-progress watchdog is 65 minutes. A lead response may issue up to six concurrent task calls as an objective resource boundary; further work is decided by the lead AI after natural results return.
- Hardened the process boundary without adding an AI controller: explicit Codex sandbox and writable paths, a minimal environment allowlist, bounded stderr, temporary-output cleanup, process-group termination, and cancellation-resistant cleanup. A real child that handled but ignored `SIGTERM` was force-killed after the 2-second grace even when the parent task was cancelled twice; no PID remained. This cleanup fits the Gateway's existing run-drain budget.
