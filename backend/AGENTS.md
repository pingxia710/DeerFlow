...
- Native task lanes preserve compact typed `HandoffEnvelope` metadata as `handoff`; it is an index hint only and must not replace the raw upstream prompt available through the task prompt or explicit refs.
- Handoff fidelity is mandatory: upstream AI raw output remains the next role's input; extracted fields are index hints, not replacements, forms, or scoring gates.
