...
Each `task` handoff writes a compact append-only audit record under the thread's audit directory when local storage is available. The record keeps status, usage, hashes, character counts, and extracted Evidence Signal fields, while omitting raw prompts, worker output, and error text.
