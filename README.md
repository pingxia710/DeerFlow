...
Account update proposals and Chair decisions are stored through `GET/POST /api/threads/{thread_id}/runs/{run_id}/account-proposals` and `POST /api/threads/{thread_id}/runs/{run_id}/account-decisions` as AI-authored governance records only; they do not automatically update Goal, Boundary, Decision, Evidence, Debt, or Learning accounts.
Chair Operating Brief is exposed through `GET /api/threads/{thread_id}/runs/{run_id}/chair-brief` with optional `round_id` and `task_id` filters; it is a short AI-readable read model over handoffs, evidence, capability facts, quality signals, review invocations, and account ledger records, not a durable decision or program-side quality judge. Phase 7 wires the same compact brief into the internal command-room lead runtime context when thread/run/round facts are available; it is not a gate, does not choose the next step, and does not dispatch reviewers or rework.
...
**Context Usage Monitor**: Each model call records a lightweight context snapshot (`estimated_tokens`, characters, message count, tool schema count, caller) before the request is sent. The UI shows the latest lead-agent context separately from cumulative token spend; raw message and tool-schema content is not copied into the snapshot.
