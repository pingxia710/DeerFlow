---
title: 沙箱
description: 沙箱为普通 Agent 和被委派的 worker 提供文件与命令环境。Command Room 不直接使用沙箱工具，而是通过一次性 Codex 子任务委派执行。
---
...
沙箱为普通 Agent profile 提供受控环境，使其可以读取文件、写入输出、运行 Shell 命令并生成产出物。Command Room profile 不同：它把路径和边界写入 `task()` prompt，交给一次性 Codex 子 AI 执行，主指挥室不直接调用沙箱文件或 Shell 工具。
...
即使没有 `bash`，普通 Agent profile 仍可以通过专用文件工具读写文件。Command Room 会把文件和 Shell 工作都通过 `task()` 委派出去。
...
`SandboxAuditMiddleware` 检查 `bash` 工具调用并记录分类后的命令审计事件。它看不到每一次专用文件工具读写，因此这是命令审计，不是完整的文件系统访问日志。
